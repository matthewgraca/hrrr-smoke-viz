...
if len(sys.argv) == 1:
    n = 30
    print('Defaulting to 30-day lookback')
elif len(sys.argv) == 2:
    try:
        n = int(sys.argv[1])
    except:
        sys.exit('int arg pls')
    print(f'Using {n}-day lookback')
else:
    sys.exit('One int arg pls')
...
for i in range(airnow.shape[0]):
    arr = n_day_lookback_avg_of(n, i, airnow)
    if len(arr != 0):
        res.append(arr)
res = np.array(res)
print(res.shape)
...
axes[1].set_title(f'{n}-day average')
...
plt.plot(dates[24:], np.mean(realigned_airnow[:, X, Y], axis=-1), label='AirNow')
plt.plot(dates[24:], np.mean(res[:, X, Y], axis=-1), label='Lookback Avg')
plt.legend()
plt.title(f'AirNow vs {n}-day lookback average: All sensors')
plt.show()

# example la sensor
plt.plot(dates[24:], realigned_airnow[:, 15, 16], label='AirNow')
plt.plot(dates[24:], res[:, 15, 16], label='Lookback Avg')
plt.legend()
plt.title(f'AirNow vs {n}-day lookback average: LA sensor')
plt.show()
