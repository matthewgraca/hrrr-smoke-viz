...
from tensorflow.keras.layers import (
    InputLayer, Input, Conv2D, Conv3D, ConvLSTM2D, Dense, Flatten, 
    Reshape, UpSampling2D, UpSampling3D, concatenate, Concatenate,
    Add, Multiply, GlobalAveragePooling2D, GlobalAveragePooling3D 
)
from libs.layers import (
    ChannelSplitLayer, TemporalCompressionLayer, 
    TileAcrossHorizonLayer, PositionalEncoding3DLayer
)
...
class DualAutoencoder:
    def __init__(
        ...
        # NOTE the goal was to support multiple architectures that end up sharing code
        # the annoying thing is that a lot of code gets ever so slightly changed, 
        # so it ends up being easier to just have a separate implementation half the
        # time :P
        self.input_shape = input_shape
        self.arch_config = arch_config
        self.convlstm_reg_config = convlstm_reg_config # hooked to nothing
    ...
    def gated_skips(self):
        def skip_gate(input_shape, prefix):
            b, h, w, c = input_shape
            inputs = Input(shape=(h, w, c), batch_size=b)
            gate = GlobalAveragePooling2D()(inputs)
            gate = Dense(c, activation='sigmoid')(gate)
            gate = Reshape((1, 1, c))(gate)
            skip_gate = Multiply()([inputs, gate])

            return Model(inputs, outputs=skip_gate, name=f'{prefix}_skip_gate')
        ...
        def spatial_decoder(x, past_encs, fcast_encs, spatial_filters, temporal_filters, strides):
            ...
            for i, (past_skip, fcast_skip) in enumerate(skips):
                ...
                past_skip = skip_gate(past_skip.shape, f'past_e_{2-i}')(past_skip)
                fcast_skip = skip_gate(fcast_skip.shape, f'fcast_e_{2-i}')(fcast_skip)
        ...
        def encoder_block(input_shape, t_filters, s_filters, strides, prefix):
            ...
            x = TemporalCompressionLayer(name=f'{prefix}_gather_last_frame')(inputs)
            x = Conv2D(
                filters=t_filters[2],
                kernel_size=(1,1), 
                padding='same', 
        ...
        def temporal_decoder(input_shapes, in_t, output_horizon, spatial_filters, temporal_filters):
            ...
            hidden_state_in = Input(shape=input_shapes[1][1:], batch_size=b, name='past_hidden_skip')
            cell_state_in = Input(shape=input_shapes[2][1:], batch_size=b, name='past_cell_skip')
            hidden_skip = skip_gate(hidden_state_in.shape, 'hidden')(hidden_state_in)
            cell_skip = skip_gate(cell_state_in.shape, 'cell')(cell_state_in)

            x_1 = TileAcrossHorizonLayer(output_horizon=output_horizon, name='dec_tile')(inputs)
            x_2 = PositionalEncoding3DLayer(output_horizon=output_horizon, name='pos_enc')(x_1)
            ...
            y = ConvLSTM2D(
                ...
                y, initial_state=[hidden_skip, cell_skip]
            )
            ...
            return Model(
                inputs=[inputs, hidden_state_in, cell_state_in],
                outputs=z, 
                name='temporal_decoder'
            )
        
        temporal_filters = self.arch_config['temporal_filters']
        spatial_filters = self.arch_config['spatial_filters']
        bottleneck = self.arch_config['bottleneck']
        latent_size = self.arch_config['latent_size']
        strides = self.arch_config['strides']

        in_t, in_h, in_w, in_c = self.input_shape 
        inputs = Input(shape=self.input_shape, batch_size=None)

        print(f"Input: {self.input_shape} -> Output: ({self.output_horizon}, {in_h}, {in_w}, 1)")
        
        # past encoder arm
        past_in = ChannelSplitLayer(self.observed_channels, name='past_channels')(inputs)
        ...
        # fcast encoder arm
        fcast_in = ChannelSplitLayer(self.forecast_channels, name='fcast_channels')(inputs)
        ...
        x = temporal_decoder([dec.shape, h.shape, c.shape], in_t, self.output_horizon, spatial_filters, temporal_filters)([dec, h, c])
    ...
    def conv3d_bn(self):
        def skip_gate(input_shape, prefix):
            ...
            inputs = Input(shape=(s, h, w, c), batch_size=b)
            gate = GlobalAveragePooling3D()(inputs)
            gate = Dense(c, activation='sigmoid')(gate)
            gate = Reshape((1, 1, 1, c))(gate)
            skip_gate = Multiply()([inputs, gate])
        ...
        def spatial_decoder(x, past_encs, fcast_encs, spatial_filters, temporal_filters, strides):
            ...
            for i, (past_skip, fcast_skip) in enumerate(skips):
                x = UpSampling3D(
                    size=(rev_strides[i], rev_strides[i], rev_strides[i]),
                    name=f'dec_up{i+1}'
                )(x)
                x = Conv3D(
                    filters=spatial_filters[-(i+2)],
            ...
            x = UpSampling3D(
                size=(2,2,2),
                name=f'dec_up{len(spatial_filters)}'
            )(x)
            x = Conv3D(temporal_filters[2], (3,3,3), padding='same', activation='relu', name=f'dec_d{len(spatial_filters)}')(x)
            return x
        ...
        def encoder_block(input_shape, t_filters, s_filters, strides, prefix):
            ...
            for i, filters in enumerate(s_filters):
                x = Conv3D(
                    filters=filters, 
                    kernel_size=(3,3,3),
                    strides=(strides[i], strides[i], strides[i]), 
                    padding='same', 
                    activation='relu', 
                    name=f'{prefix}_e{i+1}'
                )(x)
        ...
        def temporal_decoder(input_shapes, in_t, output_horizon, spatial_filters, temporal_filters):
            b, s, h, w, _c = input_shapes[0]
            inputs = Input(shape=input_shapes[0][1:], batch_size=b)
            hidden_state_in = Input(shape=input_shapes[1][1:], batch_size=b, name='past_hidden_skip')
            cell_state_in = Input(shape=input_shapes[2][1:], batch_size=b, name='past_cell_skip')

            x = PositionalEncoding3DLayer(output_horizon=output_horizon, name='pos_enc')(inputs)
            x = concatenate([x, inputs], axis=-1, name='dec_input')
            ...
            y = ConvLSTM2D(
                ...
                x, initial_state=[hidden_state_in, cell_state_in]
            )
            y = Add(name='skip_pos_enc_and_dec_lstm1')([y, inputs])
    ...
    def conv2d_bn_sigmoid_act(self):
        def skip_gate(input_shape, prefix):
            b, h, w, c = input_shape
            inputs = Input(shape=(h, w, c), batch_size=b)
            gate = GlobalAveragePooling2D()(inputs)
        ...
        output = Conv3D(1, (1,1,1), padding='same', activation='sigmoid', name='output')(x)
        
        model = Model(inputs, output)
        print(f"Architecture: {latent_size}x{latent_size} | Parameters: {model.count_params():,}")
        return model

    def dense_bn_sigmoid_act(self):
        def skip_gate(input_shape, prefix):
            b, h, w, c = input_shape
            inputs = Input(shape=(h, w, c), batch_size=b)
            gate = GlobalAveragePooling2D()(inputs)
